...
%training
W_list = zeros(num_categories, d+1);

for i=1:num_categories
    matching_indices = strcmp(categories{i,1}, train_labels);
    labels = -ones(N,1);
...
out = zeros(num_categories,1);
for i=1:M
    x = [test_image_feats(i,:) 1]';
    out =  W_list*x;
    [Y, I] = max(out);
    predicted_categories{i,1} = categories{I,1};
end
